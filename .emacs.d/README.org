     ...
     If either org-pomodoro or org-clock aren't active, print \"No Active Task \" "
      ...
      (cond ((equal :none org-pomodoro-state)
  	  ...
  	  ((equal :long-break org-pomodoro-state)
  	   (jf/org-pomodoro-waybar-string (format "Long Break • %s (%d)"  
  						  (org-pomodoro-format-seconds)
  						  org-pomodoro-count)
  					  (substring-no-properties org-clock-heading)
  					  "long-break")))))
...
#+begin_src emacs-lisp
  (use-package mu4e
    ...
    ;; mark trashed mails as trashed after marking them as read and moving to trash
    (setf (alist-get 'trash mu4e-marks)
  	'(:char ("d" . "▼")
  		:prompt "dtrash"
  		:dyn-target (lambda (target msg) (mu4e-get-trash-folder msg))
  		:action (lambda (docid msg target)
  			  ;; default is +T-N and that does not properly sync both, trash-flag plus move to trash to my dovecot IMAP server)
  			  ;; see also https://www.reddit.com/r/emacs/comments/1hzlp1h/mu4e_isync_syncing_trashed_mails/
  			  (mu4e--server-move docid
  					     (mu4e--mark-check-target target) "+S-u-N+T")))) 
...
>>>>>>> c5e1973 (emacs: org-pomodoro added, key bindings + transient menu adjusted.)
