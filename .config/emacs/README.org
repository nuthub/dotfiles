...
** eglot + eglot-java
- The built-in LSP client. It seems that more Java people are using lsp-java and not eglot (2026).
- ~project-rootfile~ can help to identify modular project sub roots. Unfortunately, ~project-rootfile~ makes tramp via SSH mostly unusable, therefore I did remove it from my config. Probably the variable ~project-vc-extra-root-markers~ and the like can also help.
