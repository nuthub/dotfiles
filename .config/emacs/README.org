  ...
  :config
    ...
    (with-eval-after-load 'ox-latex
      (add-to-list 'org-latex-classes '("moderncv"
      				      ...
      				      ("\\section{%s}" . "\\section*{%s}")
      				      ("\\subsection{%s}" . "\\subsection{%s}"))))
    ;; ox-latex: Add ceurart to the known classes
    (with-eval-after-load 'ox-latex
      (add-to-list 'org-latex-classes '("ceurart"
...
** eglot + eglot-java
+The built-in LSP client. It seems that more Java people are using lsp-java and not eglot+
...
This can probably replaced by using the variable ~project-vc-extra-root-markers~
#+begin_src emacs-lisp
